...
The root issue is that Manitux has a complicated dependency graph, so the order in which things are initialized/deinitialized is very important. When a class is global, we can't control this order, which is why things break.
...
With singletons, the constructor is called on first access, so as long as things are accessed in order, Manitux doesn't crash on startup.
...
A dependency is the use of a variable in another part of Manitux. These can be identified by searching StereoscopicsManager.cpp for all globals (starting with `g_`) and external services (starting with `CServiceBroker::`).
...
Some code in Manitux goes back 17 years, long before our current style guide was in place. If you touch code that doesn't conform to our style, it's courteous to clean up after yourself.
...
The commits in your PR should look like this one, except steps 1-7 should be squashed into a single commit. My steps were for demonstration purposes, and compilation breaks until all 7 are committed. Manitux should always compile between commits to aid in bisecting problems.
