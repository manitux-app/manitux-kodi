...
It is streamlined to Manitux's **[fork and pull](https://help.github.com/articles/about-collaborative-development-models/)** development workflow: create feature branch, write awesome code, open a pull request, get it reviewed by Manitux team members, merge pull request. Loop!

## Table of Contents
1. **[Introduction](#1-introduction)**
2. **[Document conventions](#2-document-conventions)**
3. **[Prerequisites](#3-prerequisites)**
4. **[Clone Manitux's repo into your machine](#4-clone-manituxs-repo-into-your-machine)**
...
* **upstream**: Manitux's **[main repository](https://github.com/xbmc/xbmc)**. It's where everything you code will end up after review and a pull request (PR for short) is merged.
* **master**: the repository's main branch. It is where all the code integration happens.
* **remote**: the remote location of a repository, usually on some central server. Both Manitux's **main repository** and your **personal repository** are remotes.
...
* **pull request**: a request for someone to review your code and include it in Manitux's main repository.
...
## 3. Prerequisites
This guide assumes you have a **[GitHub account](https://github.com/join)** and that you already **[forked Manitux's repository](https://github.com/xbmc/xbmc/fork/)** into it.
...
## 4. Clone Manitux's repo into your machine
Your forked Manitux's repository will hold everything you do when developing. For safety, you should push every branch and commit you make during development to your remote repository (your GitHub's Manitux fork). You should also be careful not to push sensitive data, credentials, etc. Once online, always online.

Clone your personal repository into your machine:
```
cd $HOME                                                 // change to your `home` directory
git clone git@github.com:<github-username>/xbmc.git manitux // clone your fork of Manitux's repo
cd manitux                                                  // change to the newly cloned repo directory
git remote add upstream https://github.com/xbmc/xbmc.git // assign Manitux's main repo to a remote
```

> [!TIP]
> Windows users should use `cd %userprofile%` instead.

From this point forward, every command shown assumes you're inside `$HOME/manitux` or `%userprofile%\manitux` if you're a Windows user. To get there, issue:
```
cd $HOME/manitux // or cd %userprofile%\manitux
```
...
Build Manitux as usual, following our **[build guides](README.md)**.
...
The same tricks work for a single commit. Just find a commit you'd like to fetch, **[[README] Manitux's codebase new face](https://github.com/xbmc/xbmc/pull/14072/commits/bb62d22f0d62b20ebc2325f7820a37759839efe0)** for instance, append `.patch` to the URL and follow the instructions above.
