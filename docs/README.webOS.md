...
7. **[Generate Manitux Build files](#7-Generate-Manitux-Build-files)**  
  7.1. **[Generate Project Files](#71-Generate-Project-Files)**  
  7.2. **[Add Binary Addons to Project](#72-Add-Binary-Addons-to-Project)**
8. **[Build](#8-build)**  
  8.1. **[Build manitux binary](#81-Build-manitux-binary)**
9. **[Packaging manitux to distribute as an IPK](#9-Packaging-manitux-to-distribute-as-an-IPK)**  
...
* **[Download ares CLI tools](https://webostv.developer.lge.com/develop/tools/cli-installation)**. These are required to package the manitux binary into a IPK format that webOS understands.
* **[Download and setup a compatible toolchain to compile manitux]**. The toolchain which was used in this guide is **[buildroot-nc4](https://github.com/openlgtv/buildroot-nc4)**, it is an up to date buildroot configuration and comes with newer tools such as GCC 12.2.0. Other toolchains exist, such as those found on the LG OpenSource website: **https://opensource.lge.com/**. You will need to enter the model number of your TV to find the applicable toolchain.

* Device with **webOS 5.0 or newer**.

Team Manitux CI infrastructure is limited, and therefore we only have the single combination tested. Newer webOS versions may work, however the team does not actively test/use these versions, so use with caution. Earlier versions may work, however we dont actively support them, so use with caution.
...
```
mkdir $HOME/manitux-dev
tar xzvf arm-webos-linux-gnueabi_sdk-buildroot.tar.gz -C $HOME/manitux-dev
```
...
Manitux should be built as a 32bit program for webOS. The dependencies are built in `$HOME/manitux/tools/depends` and installed into `/media/developer/apps/usr/palm/applications/org.xbmc.manitux/xbmc-deps`.
...
Configure build:
```
cd $HOME/manitux/tools/depends
./bootstrap
./configure   --prefix=$HOME/manitux-deps --host=arm-webos-linux-gnueabi \
              --with-toolchain=$HOME/manitux-dev/arm-webos-linux-gnueabi_sdk-buildroot \
              --enable-debug=no
```
...
--prefix=<path>
```
  specify path on target device. By default we will likely be packaging as org.xbmc.manitux and it will installed to $HOME/manitux/build/{org.xbmc.manitux}. All developer apps will get installed to this base path plus {your-app-name}
...
## 7. Generate Manitux Build files
Before you can build Manitux, the build files have to be generated with CMake. CMake is built as part of the dependencies and doesn't have to be installed separately. A toolchain file is also generated and is used to configure CMake.
...
Before you can build Manitux, the project has to be generated with CMake. CMake is built as part of the dependencies and doesn't have to be installed separately. A toolchain file is also generated and is used to configure CMake.
...
> [!TIP]
> BUILD_DIR can be omitted, and project will be created in $HOME/manitux/build
...
````
make -C tools/depends/target/cmakebuildsys BUILD_DIR=/media/developer/apps/usr/palm/applications/org.xbmc.manitux \
...
```
make -j$(getconf _NPROCESSORS_ONLN) -C tools/depends/target/binary-addons PREFIX=$HOME/manitux/build/tools/webOS/packaging
```

Build specific add-ons:
```
make -j$(getconf _NPROCESSORS_ONLN) -C tools/depends/target/binary-addons PREFIX=$HOME/manitux/build/tools/webOS/packaging ADDONS="audioencoder.flac pvr.vdr.vnsi audiodecoder.snesapu"
```

Build a specific group of add-ons:
```
make -j$(getconf _NPROCESSORS_ONLN) -C tools/depends/target/binary-addons PREFIX=$HOME/manitux/build/tools/webOS/packaging ADDONS="pvr.*"
```

To build specific addons or help with regular expression usage for ADDONS_TO_BUILD, view ADDONS_TO_BUILD section located at [Manitux add-ons CMake based buildsystem](../cmake/addons/README.md)

> [!TIP]
> Binary add-ons added to the generated project can be built independently of the Manitux app by selecting the scheme/target `binary-addons` in the project.

**[back to top](#table-of-contents)** | **[back to section top](#5-Generate-Manitux-Build-files)**

## 8. Build

### 8.1. Build manitux binary

In '$HOME/manitux/build`
...
## 9 Packaging manitux to distribute as an IPK

## 9.1 Create the IPK

CMake generates a target called `ipk` which will package Manitux ready for distribution.
...
Generated `ipk` file will be inside the build dir. The filename may differ from this guide which is due to the version of Manitux which at time of writing will create a `ipk` called org.xbmc.manitux_20.90.101_all.ipk.
...
```
ares-install ./org.xbmc.manitux_20.90.101_arm.ipk --d <your tv>
...
cd `/media/developer/apps/usr/palm/applications/org.xbmc.manitux/.manitux/temp` to find manitux.log

You can also edit `manitux.sh` and place strace just before launching manitux to debug any missing libraries:

```
strace -t -o /media/developer/apps/usr/palm/applications/org.xbmc.manitux/trace.temp ./manitux-webos --debug
```
...
```
cd $HOME/manitux/tools/webOS/packaging
make uninstall
```
...
```
ares-install --remove org.xbmc.manitux --d <your tv>
e.g. [LG]_webOS_TV_OLED65C24LA
```
