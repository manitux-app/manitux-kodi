...
The following is the list of packages that are used to build Manitux on Debian/Ubuntu (with all supported external libraries enabled).
...
Some packages may be missing or outdated in older distributions. Notably `crossguid`, `libfmt`, `libspdlog`, `waylandpp`, `wayland-protocols`, etc. are known to be outdated or missing. Fortunately there is an easy way to build individual dependencies with **[Manitux's unified depends build system](../tools/depends/README.md)**.
...
Some dependencies can be configured to build before Manitux. That's the case with `flatbuffers`, `crossguid`, `fmt`, `spdlog`, `rapidjson`, `fstrcmp` and `dav1d`. To enable the internal build of a dependency, append `-DENABLE_INTERNAL_<DEPENDENCY_NAME>=ON` to the configure command below. For example, configuring an X11 build with internal `fmt` would become `cmake ../manitux -DCMAKE_INSTALL_PREFIX=/usr/local -DENABLE_INTERNAL_FMT=ON` instead of `cmake ../manitux -DCMAKE_INSTALL_PREFIX=/usr/local`.
Internal dependencies that are based on cmake upstream (currently crossguid, ffmpeg, fmt, spdlog) can have their build type overridden by defining `-D<DEPENDENCY_NAME>_BUILD_TYPE=<buildtype>`. Build Type can be one of `Release, RelWithDebInfo, Debug, MinSizeRel`. eg `-DFFMPEG_BUILD_TYPE=RelWithDebInfo`. If not provided, the build type will be the same as the core Manitux project.
...
```
cmake ../manitux -DCMAKE_INSTALL_PREFIX=/usr/local -DCORE_PLATFORM_NAME=x11 -DAPP_RENDER_SYSTEM=gl
```
...
```
cmake ../manitux -DCMAKE_INSTALL_PREFIX=/usr/local -DCORE_PLATFORM_NAME=wayland -DAPP_RENDER_SYSTEM=gl
```
...
```
cmake ../manitux -DCMAKE_INSTALL_PREFIX=/usr/local -DCORE_PLATFORM_NAME=gbm -DAPP_RENDER_SYSTEM=gles
```
...
```
cmake ../manitux -DCMAKE_INSTALL_PREFIX=/usr/local -DCORE_PLATFORM_NAME="x11 wayland gbm" -DAPP_RENDER_SYSTEM=gl
```
...
> [!WARNING]  
> User running `manitux-gbm` needs to be part of `input` and `video` groups. Otherwise you'll have to use `sudo`.
...
```
sudo make install DESTDIR=$HOME/manitux
```
...
A number of addons require the the `p8-platform` and `manitux-platform` add-ons.
Note that dependencies on `p8-platform` and `manitux-platform` are typically not declared in the `depends` folder.
...
Then the manitux-platform add-on:

```
cd ~/src/
git clone https://github.com/xbmc/manitux-platform.git
cd ~/src/manitux-platform/
cmake -DCMAKE_INSTALL_PREFIX=/usr/local
make && make install
```
...
```
cd ~/src
git clone https://github.com/manitux-pvr/pvr.demo.git
cd ~/src/pvr.demo/
cmake -DCMAKE_INSTALL_PREFIX=/usr/local
make && make install
```
...
> [!NOTE]  
> `-DCMAKE_INSTALL_PREFIX=` should match Manitux's `-DCMAKE_INSTALL_PREFIX=` prefix used in **[section 4.1](#41-configure-build)**.
