...
# Android build guide
This guide has been tested with Ubuntu 16.04 (Xenial) x86_64. It is meant to cross-compile Manitux for Android using **[Manitux's unified depends build system](../tools/depends/README.md)**. Please read it in full before you proceed to familiarize yourself with the build procedure.
...
7. **[Build Manitux](#7-build-manitux)**
8. **[Package](#8-package)**
9. **[Install](#9-install)**
10. **[Debugging Manitux](#10-debugging-manitux)**
...
Install build dependencies needed to cross-compile Manitux for Android:
```
...
## 3. Prerequisites
Building Manitux for Android requires Android NDK revision 20b. For the SDK just use the latest available.
Manitux CI/CD platforms currently use r21e for build testing and releases, so we recommend using r21e for the most tested build experience
...
## 8. Package
CMake generates a target called `apk` which will package Manitux ready for distribution.
...
Generated `apk` file will be inside `$HOME/manitux`.
...
## 10. Debugging Manitux
To be able to see what is happening while running Manitux you need to enable `USB debugging` in your device settings (already enabled in the Android Emulator).
...
Launch Manitux on Android Emulator without the GUI:
```
adb shell am start -a android.intent.action.MAIN -n org.xbmc.xbmc/android.app.NativeActivity
```

Kill a misbehaving Manitux:
```
adb shell ps | grep org.xbmc | awk '{print $2}' | xargs adb shell kill
```

Filter logcat messages by a specific tag (e.g. `Manitux`):
```
adb logcat -s Manitux:V
```

Enable CheckJNI (**before** starting the Manitux):
```
adb shell setprop debug.checkjni 1
```

> [!NOTE]  
> These commands assume that current directory is `$HOME/manitux-build/tools/android/packaging` and that the proper SDK/NDK paths are set.

GDB can be used to debug, though the support is rather primitive. Rather than using `gdb` directly, you will need to use `ndk-gdb` which wraps it. You can use the `-p/--project` switches or instead you will need to `cd` to `$HOME/manitux-build/tools/android/packaging/xbmc` and execute it from there.
```
 ndk-gdb --verbose
```

This will open the installed version of Manitux and break. The warnings can be ignored as we have the appropriate paths already setup.

**[back to top](#table-of-contents)** | **[back to section top](#10-debugging-manitux)**
