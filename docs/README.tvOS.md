...
* Device with **tvOS 11.0 or newer** to install Manitux after build.
...
Manitux can be built as a 64bit program only for tvOS. The dependencies are built in `$HOME/manitux/tools/depends` and installed into `/Users/Shared/xbmc-depends`.
> [!NOTE]  
...
## 5. Generate Manitux Build files
Before you can use Xcode to build Manitux, the Xcode project has to be generated with CMake. CMake is built as part of the dependencies and doesn't have to be installed separately. A toolchain file is also generated and is used to configure CMake.
...
```
make -C tools/depends/target/cmakebuildsys BUILD_DIR=$HOME/manitux-build
```
...
````
make -C tools/depends/target/cmakebuildsys CMAKE_EXTRA_ARGUMENTS="-DPLATFORM_BUNDLE_IDENTIFIER='tv.manitux.manitux' -DCODE_SIGN_IDENTITY='iPhone Developer: *** (**********)' -DPROVISIONING_PROFILE_APP='tv.manitux.manitux' -DPROVISIONING_PROFILE_TOPSHELF='tv.manitux.manitux.Topshelf'"
````
...
> [!TIP]
> Binary add-ons added to the generated Xcode project can be built independently of the Manitux app by selecting the scheme/target `binary-addons` in the Xcode project.
You can also build the binary-addons target via xcodebuild. This will not build the Manitux App, but will build any/all binary addons added for the project Generation.
```
...
Start Xcode, open the Manitux project file created in **[Generate Manitux Build files](#5-generate-manitux-build-files)**

> [!TIP]
> (`manitux.xcodeproj`) is located in `$HOME/manitux-build`
...
This will create a `Manitux.app` file located in `$HOME/manitux-build/build/Debug-appletvos`. This App can be deployed via Xcode to an AppleTV via `Window -> Devices and Simulators -> Select device and click +`

> [!TIP]
> If you build as a release target, the location of the `Manitux.app` will be `$HOME/manitux-build/build/Release-appletvos`
...
CMake generates a target called `deb` which will package Manitux ready for distribution. After Manitux has been built, the target can be triggered by selecting it in Xcode active scheme or manually running

## 7.1. Package via Xcode

Start Xcode, open the Manitux project file created in **[Generate XCode Project Files](#51-generate-xcode-project-files)**
...
> [!TIP]
> The generated package will be located at $HOME/manitux-build/tools/darwin/packaging/tvos.
...
Note that using a free developer account the signing will need to be reapplied every 7 days.
  ...
  2. Next select the manitux build target
...
It's also important that you select the signing on all 4 spots in the project settings. After the last buildstep, our support script will do a full sign of all binaries and bundle them with the given identity, including all the `*.viz`, `*.pvr`, `*.so`, etc. files Xcode doesn't know anything about. This should allow you to deploy Manitux to all non-jailbroken devices the same way you deploy normal apps to.
In that case Manitux will be sandboxed like any other app. All Manitux files are then located in the sandboxed *Documents* folder and can be easily accessed via iTunes file sharing.

## 8.2. Using iOS App Signer to install

  1. Build the deb target via xcodebuild or Xcode as per **[Build Manitux](#6-build-manitux)**
  2. Open iOS Appsigner
  3. Browse to $HOME/manitux/build/tools/darwin/packaging/tvos for your input file
...
There are a number of different methods that can be used to install manitux on an AppleTV 4/4K.
...
Whether you have paid or free developer account you can deploy Manitux via Xcode to work on a non-jailbroken devices.
...
Once your Apple TV has been connected in Xcode, you can deploy either the **[Deb](#7-packaging-to-distribute-as-deb)** or **[App](#6-build)** file.
  ...
  3. Click the + symbol under `installed apps` and navigate to and select: `$HOME/manitux-build/build/Debug-appletvos/Manitux.app` and then `Open`.
