...
Add Manitux's *nightly* PPA to grab dependencies:
```
...
```
sudo apt build-dep manitux
```

> [!WARNING]  
> Do not use `aptitude` for the `build-dep` command. It doesn't resolve everything properly.

If at a later point you decide you do not want Manitux's PPAs on your system, removing them is as easy as:
```
...
In order to compile Manitux with AV1 support, you have to manually update both Meson and nasm.
